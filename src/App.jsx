import './App.css'
import React, { useState, useRef } from 'react';

  const initialTeam = []
  const initialMoney = 100
  const initialZombieFighters = [
    {
      name: 'Survivor',
      price: 12,
      strength: 6,
      agility: 4,
      img: 'https://via.placeholder.com/150/92c952',
    },
    {
      name: 'Scavenger',
      price: 10,
      strength: 5,
      agility: 5,
      img: 'https://via.placeholder.com/150/771796',
    },
    {
      name: 'Shadow',
      price: 18,
      strength: 7,
      agility: 8,
      img: 'https://via.placeholder.com/150/24f355',
    },
    {
      name: 'Tracker',
      price: 14,
      strength: 7,
      agility: 6,
      img: 'https://via.placeholder.com/150/d32776',
    },
    {
      name: 'Sharpshooter',
      price: 20,
      strength: 6,
      agility: 8,
      img: 'https://via.placeholder.com/150/1ee8a4',
    },
    {
      name: 'Medic',
      price: 15,
      strength: 5,
      agility: 7,
      img: 'https://via.placeholder.com/150/66b7d2',
    },
    {
      name: 'Engineer',
      price: 16,
      strength: 6,
      agility: 5,
      img: 'https://via.placeholder.com/150/56acb2',
    },
    {
      name: 'Brawler',
      price: 11,
      strength: 8,
      agility: 3,
      img: 'https://via.placeholder.com/150/8985dc',
    },
    {
      name: 'Infiltrator',
      price: 17,
      strength: 5,
      agility: 9,
      img: 'https://via.placeholder.com/150/392537',
    },
    {
      name: 'Leader',
      price: 22,
      strength: 7,
      agility: 6,
      img: 'https://via.placeholder.com/150/602b9e',
    },
  ];

const App = () => {

  const [Team, setTeam] = React.useState(initialTeam)
  const [money, setMoney] = React.useState(initialMoney)

  function handleAddFighter(fighter){
  setTeam([...Team, fighter])
  }

  function teamFunction(){
    if(Team===initialTeam){
      return "Pick some team members!"
    }else{
      return (
        Team.map((fighters, index) => {
          return <li key={index}>
          <p>Name: {fighters.name}</p>
          <p>Price: {fighters.price}</p>
          <p>Strength: {fighters.strength}</p>
          <p>Agility: {fighters.agility}</p>
          <img src={fighters.img}/>
          </li>
      }) 
    )
  }
}

  function handleMoney(fighter){
    if(money > fighter.price){
      const currentMoney = money - fighter.price
      setMoney(currentMoney)
    } else {
      console.log("Not enough money")
    }
    
  }

  return (
  <div>
    <h1>Team</h1>
      <ul>{teamFunction()}</ul>
    <h1>Zombie Fighters</h1>
      <ul id="zf">
        {initialZombieFighters.map((fighters, index) => {
          return <li key={index}>
            <button onClick={() => {handleAddFighter(fighters);handleMoney(fighters)}}>Add to team</button>
            <p>Name: {fighters.name}</p>
            <p>Price: {fighters.price}</p>
            <p>Strength: {fighters.strength}</p>
            <p>Agility: {fighters.agility}</p>
            <img src={fighters.img}/>
          </li>
        })}
      </ul>
      <p>Money: {money}</p>
  </div>
  )

}

export default App